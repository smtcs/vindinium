...
Bot.prototype.parseTheData = function (){
 ...
 // console.log(_this.yourBot.life);
  ...
  for(var i = 0; i < (tempMapSize * tempMapSize * 2); i+=2){
    ...
    if(tempMap[i] === " ") {_this.map[xPos][yPos] = " ";}
    if(tempMap[i] === "[") {_this.map[xPos][yPos] = "t"; _this.taverns.push([xPos, yPos]);}
    if(tempMap[i] === "@") {
...
Bot.prototype.pathfinder = function(pos1, pos2){
    var _this = bot;
    //console.log("pathfinder bot :" + bot);
    var pathFinderMap;
    var tempMap = _this.map;
    pathFinderMap = tempMap.reverse();
    ...
    easystar.findPath(pos1[0], pos1[1], pos2[0], pos2[1], function( path ) { 
      console.log("next spot!" + path[1].x + ", "+ path[1].y);
      if (path === null) {
